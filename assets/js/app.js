 ...
 * Updated: Edit function works without page refresh; includes small delay before view switch.
 */
...
function getFormData() {
    return {
        ...
        ownerRec: document.getElementById('f-owner-rec')?.value.trim() || '',
        ownerRepName: document.getElementById('f-owner-rep-name')?.value.trim() || '',
...
function clearForm() {
    ...
    ['f-zoning', 'f-latitude', 'f-longitude'].forEach(id => {
        const el = $(id);
        if (el) el.value = '';
    });
...
// ============================================================
// SAVE INSPECTION – Enhanced with Validation & Confirmations
// ============================================================
...
async function saveInspection() {
    ...
    // ============================================================
    // STEP 1: Validate Entity Details
    // ============================================================
    const name = document.getElementById('f-name')?.value.trim() || '';
    const date = document.getElementById('f-date')?.value || '';
    
    if (!name) {
        showToast('⚠️ Please enter the entity name', 'error');
        document.getElementById('f-name')?.focus();
        return;
    }
    
    if (!date) {
        showToast('⚠️ Please select an inspection date', 'error');
        document.getElementById('f-date')?.focus();
        return;
    }
    
    // ============================================================
    // STEP 2: Validate All Checklist Items Are Answered
    // ============================================================
    const items = document.querySelectorAll('.checklist-item');
    ...
    items.forEach(item => {
        const answered = item.querySelector('.status-btn.on');
        if (!answered) {
            allAnswered = false;
            if (!firstUnanswered) firstUnanswered = item;
            item.style.borderLeft = '3px solid var(--cok-red)';
    ...
    if (!allAnswered) {
        ...
        showToast(`⚠️ Please answer all checklist items (${remaining} remaining)`, 'error');
        if (firstUnanswered) {
            firstUnanswered.scrollIntoView({ behavior: 'smooth', block: 'center' });
            firstUnanswered.style.backgroundColor = 'var(--red-bg)';
            setTimeout(() => {
                firstUnanswered.style.backgroundColor = 'transparent';
            }, 2000);
        }
        return;
    }
    ...
    // ============================================================
    // STEP 3: Ask "Save this inspection?"
    // ============================================================
    const confirmSave = confirm('💾 Do you want to save this inspection?');
    if (!confirmSave) {
    ...
    try {
        ...
        Object.keys(data.answers).forEach(key => {
            ...
            if (!isNaN(dbId) && dbId > 0) {
                formattedAnswers[dbId] = { 
                    status: data.answers[key].status || '', 
                    comment: data.answers[key].comment || '' 
                };
            }
        });
        ...
        if (result.success) {
            ...
            if (document.getElementById('form-heading')) {
                document.getElementById('form-heading').textContent = `Editing — ${data.name}`;
            }
            await loadInspections();
            await loadDashboard();
            
            // ============================================================
            // STEP 5: Ask "View report?"
            // ============================================================
            const viewReport = confirm('📄 Inspection saved! Would you like to view the report?');
            if (viewReport) {
                window.open(`reports/report.php?id=${result.inspectionId}`, '_blank');
            } else {
                if (confirm('Would you like to go to the records page?')) {
...
// ============================================================
// editInspection – FIXED: No page reload, populates form directly
// ============================================================
...
async function editInspection(id) {
    ...
    state.currentRecord = { id: insp.id };
    
    // ============================================================
    // STEP 1: Build answers map and set state.answers FIRST
    // This ensures loadChecklist() renders with the correct statuses
    // ============================================================
    const answers = {};
    data.answers.forEach(a => { answers[a.item_id] = { status: a.status || '', comment: a.comment || '' }; });
    state.answers = answers;  // CRITICAL: Set BEFORE loadChecklist
    ...
    // ============================================================
    // STEP 2: Populate Entity Details
    // ============================================================
    ...
    // Set zoning, latitude, longitude
    const zoningEl = document.getElementById('f-zoning');
    if (zoningEl) zoningEl.value = record.zoning || '';
    const latEl = document.getElementById('f-latitude');
    if (latEl) latEl.value = record.latitude || '';
    const lngEl = document.getElementById('f-longitude');
    if (lngEl) lngEl.value = record.longitude || '';
    
    // Set date
    const dateEl = document.getElementById('f-date');
    if (dateEl) dateEl.value = record.date || new Date().toISOString().slice(0, 10);
    
    // Set entity type hidden field
    const entityTypeEl = document.getElementById('f-entity-type');
    if (entityTypeEl) entityTypeEl.value = record.entityType;
    
    state.entityType = record.entityType;
    
    // ============================================================
    // STEP 3: Load Checklist – It will use state.answers
    // ============================================================
    await loadChecklist(record.entityType);
    
    // ============================================================
    // STEP 4: Populate checklist answers (status buttons and comments)
    // ============================================================
    const checklistItems = document.querySelectorAll('.checklist-item');
    checklistItems.forEach(row => {
        const itemId = parseInt(row.dataset.itemId);
        const ans = record.answers[itemId];
        if (ans) {
            const btns = row.querySelectorAll('.status-btn');
            btns.forEach(btn => {
                btn.classList.toggle('on', btn.dataset.v === ans.status);
            });
            const commentInput = row.querySelector('.item-comment');
    ...
    // ============================================================
    // STEP 5: Populate Team
    // ============================================================
    const teamContainer = document.getElementById('team-rows');
    if (teamContainer) {
    ...
    if (heading) heading.textContent = `Editing — ${record.name}`;
    
    // Update stats
    updateStats();
    
    // ============================================================
    // STEP 8: Switch to form view with a small delay to allow DOM rendering
    // ============================================================
    setTimeout(() => {
        switchView('form');
        // Update URL without reload
        const newUrl = `?view=${record.entityType === 'petrol' ? 'petrol' : 'building'}&edit=${record.id}`;
        window.history.pushState({}, '', newUrl);
        showToast('📝 Loaded inspection for editing', 'success');
    }, 150);
}
...
function switchView(view) {
    ...
    if (view === 'dashboard') {
        ...
        // If editing, checklist is already loaded
    } else if (view === 'records') {
        const recordsView = document.getElementById('view-records');
        if (recordsView) recordsView.classList.add('active');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (view === 'dashboard') {
        ...
        // If editId is present, load the inspection data
        if (editId > 0) {
            // Need to wait for the checklist container to be ready
            setTimeout(() => {
